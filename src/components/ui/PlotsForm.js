...
import PlotTypeInput from "./PlotTypeInput"
import PowerSupplyInput from "./PowerSupplyInput"
import CommentsInput from "./CommentsInput"
import ConditionsInput from "./ConditionsInput"

const PlotsForm = () => {
  const name = "parceles-form"
      ...
      validate={values => {
        ...
        if (values.dataArribada && values.dataSortida) {
          if (new Date(values.dataArribada) >= new Date(values.dataSortida))
            errors.dataArribada = errorDates
        }
      ...
      {props => (
        <Form name={name} data-netlify="true" data-netlify-honeypot="bot-field">
          ...
          >
            ...
            <GridItem colStart={[1, null, 2]} rowStart={[7, null, 2]}>
              <HStack spacing={4}>
                <Field name="tipusParcela">
                  {({ field, form }) => (
                    <PlotTypeInput
                      field={field}
                      placeholderStyles={placeholderStyles}
